...
namespace rr
{
	...
	void RrConfig::Trim(std::string& str)
	{
		...
		end_pos = i;
		str = str.substr(start_pos, static_cast<std::basic_string<char, std::char_traits<char>, std::allocator<char>>::size_type>(end_pos) - start_pos + 1);
	}
	...
	bool RrConfig::AnalyseLine(const std::string& line, std::string& section, std::string& key, std::string& value)
	{
		...
		{
			section = line.substr(static_cast<std::basic_string<char, std::char_traits<char>, std::allocator<char>>::size_type>(s_startpos) + 1, static_cast<std::basic_string<char, std::char_traits<char>, std::allocator<char>>::size_type>(s_endpos) - 1);
			return true;
		}
		std::string new_line = line.substr(start_pos, static_cast<std::basic_string<char, std::char_traits<char>, std::allocator<char>>::size_type>(start_pos) + 1 - end_pos);
		if ((pos = new_line.find('=')) == -1)
			return false;
		key = new_line.substr(0, pos);
		value = new_line.substr(static_cast<std::basic_string<char, std::char_traits<char>, std::allocator<char>>::size_type>(pos) + 1, static_cast<std::basic_string<char, std::char_traits<char>, std::allocator<char>>::size_type>(end_pos) + 1 - (static_cast<unsigned long long>(pos) + 1));
		Trim(key);
		if (key.empty()) {
			return false;
		}
	...
	int RrConfig::ReadIntEx(const char* section, const char* item, const int& default_value, int& illegalCharPosition) {
		std::string tmp_s(section);
		std::string tmp_i(item);
		std::map<std::string, std::string> k_v;
		std::map<std::string, std::string>::iterator it_item;
		...
		it = settings_.find(tmp_s);
		if (it == settings_.end()) {
			illegalCharPosition = -1; // 表示没有找到 section
			return default_value;
		}
		k_v = it->second;
		it_item = k_v.find(tmp_i);
		if (it_item == k_v.end()) {
			illegalCharPosition = -1; // 表示没有找到 item
			return default_value;
		}

		// 检查字符串中是否有非法字符
		size_t pos = 0;
		for (char c : it_item->second) {
			if (!isdigit(c)) { // 假设非法字符是非数字字符
				illegalCharPosition = static_cast<int>(pos);
				return default_value; // 返回默认值并报告非法字符位置
			}
			++pos;
		}

		// 如果没有非法字符，正常转换并返回
		illegalCharPosition = -1; // 重置非法字符位置
		return atoi(it_item->second.c_str());
	}
